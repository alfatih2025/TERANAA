...
export function resolveBmkgForecastCode(code: unknown) {
  const normalizedCode = normalizeWeatherLocationCode(code);
  const location = getWeatherLocationByCode(normalizedCode);

  if (location?.level !== 'district') return normalizedCode;

  return (
    WEATHER_LOCATION_OPTIONS.find(
      (item) => item.level === 'village' && item.parentCode === normalizedCode,
    )?.code || normalizedCode
  );
}

export function buildBmkgWeatherUrl(baseUrl: string, locationCode?: string) {
  const normalizedCode = resolveBmkgForecastCode(locationCode);
