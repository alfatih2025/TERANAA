    ...
    configureServer(server: any) {
      server.middlewares.use(async (req: any, res: any, next: any) => {
        ...
        if (apiName === 'openrouter') {
          try {
            // Dynamic import so this module is never loaded during `vite build`
            const { getOpenRouterStatus, sendOpenRouterMessage } = await import(
              // @ts-expect-error - internal helper
              './src/lib/apiHelpers/_openrouter.js'
            );
