...
export function ChatInterface({ sensorData = null, settings = null, weatherData = null, variant = 'full' }: ChatInterfaceProps) {
  ...
  const rootClassName = isCompact
    ? 'flex flex-col overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-950'
    : 'flex h-[calc(100vh-200px)] min-h-[520px] flex-col overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-950';

  const headerClassName = isCompact
    ? 'rounded-t-2xl border-b border-gray-100 bg-gradient-to-r from-green-50 to-teal-50 p-3 dark:border-slate-800 dark:from-slate-900 dark:to-slate-950'
    : 'rounded-t-2xl border-b border-gray-100 bg-gradient-to-r from-green-50 to-teal-50 p-4 dark:border-slate-800 dark:from-slate-900 dark:to-slate-950';

  const bodyClassName = isCompact ? 'space-y-3 overflow-y-auto p-3' : 'space-y-4 overflow-y-auto p-4 flex-1';

  const footerClassName = isCompact ? 'border-t border-gray-100 bg-white p-3 dark:border-slate-800 dark:bg-slate-950' : 'border-t border-gray-100 bg-white p-4 dark:border-slate-800 dark:bg-slate-950';
  ...
  return (
    <div className={rootClassName}>
      <div className={headerClassName}>
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className={`font-semibold text-gray-800 dark:text-slate-100 ${isCompact ? 'text-sm' : ''}`}>TERANA AI</h3>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={() => {
                if (window.confirm('Hapus semua riwayat chat hari ini? Histori tanggal lain tetap aman.')) {
                  void clearMessages();
                }
              }}
              disabled={loading || messages.length === 0}
              className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-red-700 transition hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-50 dark:border-red-500/30 dark:bg-red-950/40 dark:text-red-300 dark:hover:bg-red-950/60"
              title="Hapus riwayat chat hari ini"
            >
              Hapus Chat
            </button>
            <button
              ...
              disabled={loading}
              className="rounded-lg border border-white/60 bg-white/80 p-2 text-gray-600 transition hover:bg-white light-card disabled:opacity-50 dark:border-slate-700 dark:bg-slate-800/80 dark:text-slate-300 dark:hover:bg-slate-700"
        ...
        {!isCompact && (
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            <div className="rounded-xl border border-green-100 bg-white/80 p-3 dark:border-green-500/20 dark:bg-slate-900/80">
      ...
      <div className={bodyClassName}>
        ...
        {messages.length === 0 && (
          <div className={isCompact ? 'rounded-2xl border border-green-100 bg-green-50/60 p-4 text-center dark:border-green-500/20 dark:bg-green-950/20' : 'py-10 text-center'}>
            <div className={`mx-auto mb-4 flex items-center justify-center rounded-full bg-green-50 dark:bg-green-950/40 ${isCompact ? 'h-14 w-14' : 'h-20 w-20'}`}>
              <Leaf className={`text-green-600 ${isCompact ? 'h-7 w-7' : 'h-10 w-10'}`} />
            </div>
            <h4 className={`mb-2 font-semibold text-gray-700 dark:text-slate-200 ${isCompact ? 'text-sm' : 'text-lg'}`}>Selamat datang di TERANA AI!</h4>
            <p className={`mx-auto dark:text-slate-400 ${isCompact ? 'mb-4 text-xs' : 'mb-6 max-w-md text-gray-600'}`}>
              ...
              {quickPrompts.map((prompt) => {
                ...
                return (
                  <button
                    ...
                    className={`inline-flex items-center gap-2 rounded-full border border-green-100 bg-green-50 font-medium text-green-700 transition hover:bg-green-100 dark:border-green-500/20 dark:bg-green-950/30 dark:text-green-300 dark:hover:bg-green-950/50 ${isCompact ? 'px-3 py-1.5 text-xs' : 'px-4 py-2 text-sm'}`}
                  >
                    <Icon size={14} />
                    {prompt.label}
                  </button>
        ...
        {loading && (
          <div className="flex justify-start">
            <div className="max-w-[80%] rounded-2xl bg-gray-100 px-4 py-3 text-sm text-gray-600 shadow-sm dark:bg-slate-800 dark:text-gray-300">
        ...
        {error && (
          <div className="flex justify-start">
            <div className="max-w-[80%] rounded-2xl border border-red-100 bg-red-50 px-4 py-3 text-sm text-red-700 shadow-sm dark:border-red-500/30 dark:bg-red-950/40 dark:text-red-300">
              {error}
            </div>
          </div>
        )}
      ...
      <form onSubmit={handleSubmit} className={footerClassName}>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-2">
          <textarea
            ...
            rows={isCompact ? 1 : 2}
            className={`flex-1 resize-none rounded-xl border border-gray-200 bg-slate-100 px-4 py-3 text-sm text-slate-900 placeholder:text-slate-500 focus:border-green-500 focus:outline-none focus:ring-2 focus:ring-green-500/20 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:placeholder:text-slate-500 ${
