  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900 sm:p-6">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {phaseOptions.map((phase) => (
              <motion.button
                ...
                className={`min-h-28 rounded-xl border-2 p-4 text-left transition-all dark:bg-gray-950/40 ${!isAdmin ? 'cursor-not-allowed opacity-70' : ''} ${currentPhase === phase.id ? 'border-green-500 bg-green-50 dark:bg-green-950/30' : 'border-gray-200 hover:border-green-200 dark:border-gray-700 dark:hover:border-green-500/60'}`}
              >
                <span className="mb-2 block text-2xl">{phase.icon}</span>
                <span className={`font-medium ${currentPhase === phase.id ? 'text-green-700 dark:text-green-300' : 'text-gray-700 dark:text-gray-200'}`}>
                  {phase.id === 'vegetatif' ? 'Vegetatif' : 'Generatif'}
                </span>
              </motion.button>
            ))}
          </div>
          <div className="mt-5 rounded-xl border border-green-100 bg-green-50 p-4 text-sm text-green-900 dark:border-green-500/20 dark:bg-green-950/25 dark:text-green-100">
            <p className="font-semibold">{phaseProfile.label}</p>
            <p className="mt-1 text-green-800 dark:text-green-200">{phaseProfile.description}</p>
            <p className="mt-2 text-green-700 dark:text-green-300">Rentang rekomendasi tanah: {formatRange(phaseProfile.soilRange)}</p>
            <p className="mt-1 text-green-700 dark:text-green-300">Rentang rekomendasi kelembapan udara: {phaseProfile.humidityRange[0]}%–{phaseProfile.humidityRange[1]}%</p>
          </div>
