...
const WEATHER_CACHE_PREFIX = 'nexagrow_weather_cache_v4';
...
function readWeatherCache(locationCode: string) {
  ...
  try {
    ...
    if (normalizeWeatherLocationCode(cacheRecord.weather.location_code) !== locationCode) return null;
...
function normalizeWeatherData(locationCode: string, data: WeatherData | null | undefined, fallbackLabel: string, pathLabel: string): WeatherData {
  ...
  return {
    ...
    current: {
      ...
      rain_amount: Number(current.rain_amount) || 0,
    },
    forecast,
  };
}
...
async function readJsonResponse(response: Response, label: string) {
  ...
  if (!contentType.toLowerCase().includes('application/json')) {
    throw new Error(`${label} mengembalikan respons bukan JSON (${response.status})`);
  }

  try {
    return JSON.parse(body) as unknown;
  } catch {
    throw new Error(`${label} mengembalikan JSON tidak valid`);
  }
}

export function useWeather(locationCode?: string | null) {
  ...
  useEffect(() => {
    ...
    prevLocationCode.current = normalizedLocation;

    const isCurrentRequest = () => requestId.current === currentRequestId && !controller.signal.aborted;

    setLoading(true);
    setError(null);
    // Do not render cached or synthetic weather while a new location is loading.
    // Those values can briefly show the wrong conditions before BMKG responds.
    setData(null);

    const fetchWeather = async () => {
      try {
        const apiUrl = `/api/weather/${encodeURIComponent(normalizedLocation)}?location=${encodeURIComponent(normalizedLocation)}&label=${encodeURIComponent(pathLabel)}`;
        const response = await fetch(apiUrl, { signal: controller.signal, cache: 'no-store' });

        if (response.ok) {
          const weather = await readJsonResponse(response, 'API cuaca');
        ...
        if (BMKG_URL) {
          ...
          const bmkgRes = await fetch(bmkgUrl, { signal: controller.signal, cache: 'no-store' });

          if (bmkgRes.ok) {
            const bmkgJson = await readJsonResponse(bmkgRes, 'BMKG');
        ...
        if (!isCurrentRequest() || (err as Error)?.name === 'AbortError') return;
  ...
  return {
    data: locationCode === null ? null : data,
    loading: locationCode === null || loading,
    error: locationCode === null ? null : error,
  };
}
