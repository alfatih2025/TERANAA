...
function normalizeAiModel(provider, model, fallback) {
  ...
  if (provider === 'groq' && ['llama-3.1-8b-instant', 'llama-3.3-70b-versatile', 'mixtral-8x7b-32768'].includes(normalized)) return CURRENT_GROQ_MODEL;
  if (provider !== 'gemini') return normalized;
  if (['gemini-2.0-flash', 'gemini-2.5-flash-lite', 'gemini-2.5-flash-lite-preview', 'gemini-3.1-pro', 'gemini-3.6-flash'].includes(normalized)) return CURRENT_GEMINI_MODEL;
  return normalized;
}
